def test_register_login_and_create_task(auth_client, task_client, registered_user):
    ...
    resp = task_client.post("/api/tasks/", json=task_payload, headers=headers)
    ...
    resp = task_client.get("/api/tasks/", headers=headers)
...
def test_task_missing_user_id(auth_client, task_client, registered_user):
    ...
    resp = task_client.post("/api/tasks/", json=task_payload)
...
def test_complete_task_flow(auth_client, task_client, registered_user):
    ...
    resp = task_client.post(f"/api/tasks/{task['id']}/complete", headers=headers)
    assert resp.status_code == 200
